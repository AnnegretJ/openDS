...
public class CollideWithTriggerCondition extends TriggerCondition
{
	...
	public CollideWithTriggerCondition(SceneLoader sceneLoader, String triggerName, String modelID)
	{
		super(triggerName);
	...
	public void evaluate(Simulator sim, int priority, ArrayList<TriggerAction> triggerActionList)
	{
		...
		for(MapObject mapObject : sceneLoader.getMapObjects())
			if(mapObject.getName().equals(modelID))
				modelIDExists = true;
		
		if(modelIDExists)
			SimulationBasics.getTriggerActionListMap().put(modelID, triggerActionList);
		else
			System.err.println("Trigger '" + triggerName + "' has invalid <collideWith> condition (modelID '" 
