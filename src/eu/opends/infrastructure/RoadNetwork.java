...
public class RoadNetwork 
{
    ...
    private boolean alignWaypoints;
	...
	public RoadNetwork(Simulator sim)
	{
		...
		// set up alignment and visualization of way points and segments
		alignWaypoints = scenarioLoader.isAlignRoadWaypoints();
		drawWaypoints = scenarioLoader.isVisualizeRoadWaypoints();
	    drawSegments = scenarioLoader.isVisualizeRoadSegments();
		
		if(alignWaypoints)
		{
			// align vertical position of each way point to terrain (--> 1 m above terrain)
			for(Waypoint waypoint : waypointMap.values())
			{
				float x = waypoint.getPosition().getX();
				float z = waypoint.getPosition().getZ();
				
				float y = getHeightAt(x, z) + 1.0f;
				waypoint.getPosition().setY(y);
			}
		}
