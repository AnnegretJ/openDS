...
public abstract class Car
{
    ...
    protected Node invisibleCarNode; 	// sub node of scene node
    protected Node carNode;				// sub node of invisible car node
    ...
    protected float acceleratorPedalIntensityByUser;
    protected float brakePedalIntensityByUser;
    protected float clutchPedalIntensityByUser;
    ...
    protected void init()
    {
		...
		carControl.init(this, maxBrakeForce, maxFreeWheelBrakeForce);
		invisibleCarNode = carModel.getInvisibleCarNode();
        ...
        if(carControl.isUseChrono())
        	// initial position and rotation already set in ChronoVehicleControl
        ...
        else
        {
            ...
            // add trailer (bullet only)
            boolean hasTrailer = false;
            if(hasTrailer)
            	setupTrailer();

            // set initial position and orientation (bullet only)
            setPositionRotation(initialPosition, initialRotation);
        }
	...
	public Node getInvisibleCarNode()
	{
		return invisibleCarNode;
	}
	...
	public void setToResetPosition(int keyNumber) 
	{
		...
		if (keyNumber < numberOfResetPoints) 
		{
			...
			setPositionRotation(location, rotation);
		}
	}
	...
	public void setPositionRotation(Vector3f pos, Quaternion rot) 
	{
		...
		carControl.setPhysicsLocationRotation(previousPosition, rotation);

		
		if(trailerControl != null)
			trailerControl.setPhysicsLocationRotation(previousPosition, rotation);
	}
	...
	{
		acceleratorPedalIntensityByUser = intensity;
	}

	
	public float getAcceleratorPedalIntensity() 
	{
		return Math.abs(acceleratorPedalIntensityByUser);
	}
	...
	{
		brakePedalIntensityByUser = intensity;
	...
	{
		return brakePedalIntensityByUser;
	}
	...
	{		
		clutchPedalIntensityByUser = intensity;
	}
	
	
	public float getClutchPedalIntensity() 
	{
		return clutchPedalIntensityByUser;
	}

	
	float previousClutchPedalIntensityByUser = 0;
	public float getTraction() 
	{
		if(clutchPedalIntensityByUser < 0.4f && isEngineOn())
		{
			if(FastMath.abs(previousClutchPedalIntensityByUser-clutchPedalIntensityByUser) < 0.05f)
				traction = Math.min(1.0f, traction + 0.0005f);
			else
				traction = (0.4f - clutchPedalIntensityByUser)*2.5f;
		}
		else
			traction = 0;
		
		previousClutchPedalIntensityByUser = clutchPedalIntensityByUser;
	...
	public void resetPedals()
	{
		...
		acceleratorPedalIntensityByUser = 0;
		brakePedalIntensityByUser = 0;
	}
	...
	public float getCurrentSpeedKmh()
	{
		...
		//return Math.max(-carControl.getCurrentVehicleSpeedKmHour(),0);
