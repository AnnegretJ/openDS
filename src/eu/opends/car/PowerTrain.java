...
public class PowerTrain 
{
	...
	private boolean isIdleEngine(float fuelInLiters) 
	{
		...
		return (fuelInLiters == 0) && 
					(car.getCarControl().getRPM() <= car.getCarControl().getMinRPM()) &&
					car.isEngineOn();
	}
	...
	{
		float rpm = car.getCarControl().getRPM();
	...
	private float[] computeLitersPerX()
	{
        ...
        if((car.getCarControl().getGear()!=0) && (car.getCurrentSpeedKmh() > 1))
        {
        	// liters per 100 km
	...
	private float getPEngine(float gasPedalPressIntensity)
	{
		...
		float rotationsPerMinute = car.getCarControl().getRPM();
...
*/	
	...
	private float getPGrade(float velocity, float vehicleMass, float gravityConstant)
	{
		...
		if(car.getCarControl().getGear() == -1)
			angle = -angle;
