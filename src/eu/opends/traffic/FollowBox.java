...
public class FollowBox 
{
    ...
    // Specifies whether a traffic object will be reset to its initial way point if there is 
    // it has not moved for more than 30 seconds.
    private boolean resetWhenStuck = true;
	...
	{		
		...
		resetWhenStuck = settings.getResetWhenStuck();
	...
	private void performWayPointChange(Waypoint waypoint)
	{
		...
		if(success && (!(trafficObject instanceof SteeringCar) || ((SteeringCar)trafficObject).isAutoPilot()))
		{
			...
			// get heading to traffic object
			if(currentSegment != null)
			{
				float heading = currentSegment.getHeading(traveledDistance);
				quaternion = new Quaternion().fromAngles(0, heading, 0);
			}
			
			// set position (and heading) to traffic object
			trafficObject.setPositionRotation(waypoint.getPosition(), quaternion);
		}
	}
	...
	private boolean prioritizedTrafficAhead()
	{
		...
		if(!prioritizedSegments.isEmpty() && distanceToNextWP < ownSafetyDistanceToIntersection)
		{
			...
			for(TrafficObject otherTrafficObject : PhysicalTraffic.getTrafficObjectList())
