			...
			for (int k = 1; k <= modelNodes.getLength(); k++) 
			{
				...
				for (int j = 1; j <= childnodes.getLength(); j++) 
				{
					...
					else if(currentChild.getNodeName().equals("openDrivePosition"))
					{
						...
						String roadID_str = childAttributes.getNamedItem("roadID").getNodeValue();
						if(roadID_str != null && !roadID_str.isEmpty())
							roadID = roadID_str;
						
						String s_str = childAttributes.getNamedItem("s").getNodeValue();
						if(s_str != null && !s_str.isEmpty())
							s = Float.parseFloat(s_str);
						
						String lateralOffset_str = childAttributes.getNamedItem("lateralOffset").getNodeValue();
						if(lateralOffset_str != null && !lateralOffset_str.isEmpty())
							lateralOffset = Float.parseFloat(lateralOffset_str);
						
						String verticalOffset_str = childAttributes.getNamedItem("verticalOffset").getNodeValue();
						...
						(collisionShape != null))
				{
					...
					mapObjectsList.add(mapObject);
						...
						(lateralOffset != null) && (rotation != null) && (collisionShape != null))
				{
