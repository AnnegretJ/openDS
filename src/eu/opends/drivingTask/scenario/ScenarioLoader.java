...
public class ScenarioLoader 
{
	...
	private boolean alignRoadWaypoints = false;
	...
	private void extractOpenDriveEgoCarData()
	{

		try {
			
			startRoadID = dtData.getValue(Layer.SCENARIO, 
					"/scenario:scenario/scenario:driver/scenario:openDrive/scenario:startRoadID", String.class);
			
			startLane = dtData.getValue(Layer.SCENARIO, 
					"/scenario:scenario/scenario:driver/scenario:openDrive/scenario:startLane", Integer.class);
			
			startS = dtData.getValue(Layer.SCENARIO, 
					"/scenario:scenario/scenario:driver/scenario:openDrive/scenario:startS", Double.class);
			
			targetRoadID = dtData.getValue(Layer.SCENARIO, 
					"/scenario:scenario/scenario:driver/scenario:openDrive/scenario:targetRoadID", String.class);
			
			targetLane = dtData.getValue(Layer.SCENARIO, 
					"/scenario:scenario/scenario:driver/scenario:openDrive/scenario:targetLane", Integer.class);
			...
			for (int k = 1; k <= laneNodes.getLength(); k++) 
			{
				String junctionID = dtData.getValue(Layer.SCENARIO, path + "["+k+"]/@junctionID", String.class);
				String connectionID = dtData.getValue(Layer.SCENARIO, path + "["+k+"]/@connectionID", String.class);
				
				if(junctionID != null && !junctionID.isEmpty() && connectionID != null && !connectionID.isEmpty())
				{
					//System.err.println("junctionID: " + junctionID + "; connectionID:" + connectionID);
	...
	private void extractTraffic()
	{
		...
		try {
			...
			for (int k = 1; k <= pedestrianNodes.getLength(); k++) 
			{
				...
				PedestrianData pedestrianData = new PedestrianData(name, enabled, mass, animationStand, 
						animationWalk, localScale, localTranslation, localRotation, modelPath, followBoxSettings);
		...
		try {
			NodeList oDvehicleNodes = (NodeList) dtData.xPathQuery(Layer.SCENARIO, 
					"/scenario:scenario/scenario:traffic/scenario:ODvehicle", XPathConstants.NODESET);

			for (int k = 1; k <= oDvehicleNodes.getLength(); k++) 
			{
				Node currentNode = oDvehicleNodes.item(k-1);
					
				//String name = dtData.getValue(Layer.SCENARIO, 
				//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/@id", String.class);
				String name = currentNode.getAttributes().getNamedItem("id").getNodeValue();
				...
				for (int j = 1; j <= childnodes.getLength(); j++) 
				{
					...
					//Float mass = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:mass", Float.class);
					if(currentChild.getNodeName().equals("mass"))
					{
						mass = Float.parseFloat(currentChild.getTextContent());
					}
					...
					//Float acceleration = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:acceleration", Float.class);
					else if(currentChild.getNodeName().equals("acceleration"))
					{
						acceleration = Float.parseFloat(currentChild.getTextContent());
					}
					...
					//Float decelerationBrake = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:decelerationBrake", Float.class);
					else if(currentChild.getNodeName().equals("decelerationBrake"))
					{
						decelerationBrake = Float.parseFloat(currentChild.getTextContent());
					}
					...
					//Float decelerationFreeWheel = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:decelerationFreeWheel", Float.class);
					else if(currentChild.getNodeName().equals("decelerationFreeWheel"))
					{
						decelerationFreeWheel = Float.parseFloat(currentChild.getTextContent());
					}
					...
					//Boolean engineOn = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:engineOn", Boolean.class);
					else if(currentChild.getNodeName().equals("engineOn"))
					{
						engineOn = Boolean.parseBoolean(currentChild.getTextContent());
					}
					...
					//String modelPath = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:modelPath", String.class);
					else if(currentChild.getNodeName().equals("modelPath"))
					{
						modelPath = currentChild.getTextContent();
					}	
					...
					//Boolean isSpeedLimitedToSteeringCar = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:neverFasterThanSteeringCar", Boolean.class);
					else if(currentChild.getNodeName().equals("neverFasterThanSteeringCar"))
					{
						isSpeedLimitedToSteeringCar = Boolean.parseBoolean(currentChild.getTextContent());
					}
					...
					//Float decelerationBrake = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:distanceFromPath", Float.class);
					else if(currentChild.getNodeName().equals("distanceFromPath"))
					{
					...
					//String modelPath = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:startRoadID", String.class);
					else if(currentChild.getNodeName().equals("startRoadID"))
					{
						startRoadID = currentChild.getTextContent();
					}	
					
					//Integer decelerationBrake = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:startLane", Integer.class);
					else if(currentChild.getNodeName().equals("startLane"))
					{
						startLane = Integer.parseInt(currentChild.getTextContent());
					}
					
					//Double decelerationFreeWheel = dtData.getValue(Layer.SCENARIO, 
					//		"/scenario:scenario/scenario:traffic/scenario:ODvehicle["+k+"]/scenario:startS", Double.class);
					else if(currentChild.getNodeName().equals("startS"))
					{
					...
					else if(currentChild.getNodeName().equals("preferredConnections"))
					{
						...
						for (int l=1; l<=ODconnectionNodes.getLength(); l++) 
						{
							Node ODconnectionNode = ODconnectionNodes.item(l-1);
							
							if(ODconnectionNode != null && ODconnectionNode.getNodeName() != null &&
									ODconnectionNode.getNodeName().equals("ODconnection"))
							{
								String junctionID = ODconnectionNode.getAttributes().getNamedItem("junctionID").getNodeValue();
								String connectionID = ODconnectionNode.getAttributes().getNamedItem("connectionID").getNodeValue();
								preferredConnections.addConnection(junctionID, connectionID);
							}
						}
					}
				...
				for (int k = 1; k <= segmentNodeList.getLength(); k++) 
				{
					...
					if(currentSegmentNode.getNodeName().equals("segment"))
					{
						...
						if((id != null) && (from != null) && (to != null))
						{
							...
							throw new Exception("Error in segment list");
					}
				}
	...
	{	
		...
		{			
			try {
				...
				if(wayPointsNode.getAttributes().getNamedItem("alignToTerrain") != null)
					alignRoadWaypoints = Boolean.parseBoolean(wayPointsNode.getAttributes().getNamedItem("alignToTerrain").getNodeValue());
		...
		try {
			...
			for (int i = 1; i <= intersectionNodes.getLength(); i++) 
			{
				...
				for (int k = 1; k <= trafficLightNodes.getLength(); k++) 
				{
					...
					// look up whether OpenDRIVE position available
					OffroadPositionType openDriveposition = null;
					for(MapObjectOD mapObjectOD : Simulator.getDrivingTask().getSceneLoader().getDependentMapObjects())
					{
						if(trafficLightID.equals(mapObjectOD.getName()))
						{
							openDriveposition = new OffroadPositionType();
							openDriveposition.setSegment(mapObjectOD.getRoadID());
							openDriveposition.setS(mapObjectOD.getS());
							openDriveposition.setLateralOffset(mapObjectOD.getLateralOffset());
						}
					}
					...
					// get OpenDRIVE lane number affected by the traffic light
					Integer affectedLane = dtData.getValue(Layer.SCENARIO, 
							path + "[" + i + "]/scenario:trafficLights/scenario:trafficLight[" + k + "]/scenario:affectedLane", Integer.class);					
					
					// create new traffic light
					TrafficLight trafficLight = new TrafficLight((Simulator) sim, trafficLightID, openDriveposition, affectedLane, trafficLightTriggerID, 
							trafficLightPhaseTriggerID, intersectionID, initialState, direction, phasePosition, requiresRedList, positionData);
	...
	private FollowBoxSettings extractFollowBoxSettings(Node node) 
	{
		...
		try {
			...
			for (int j = 1; j <= childnodes.getLength(); j++) 
			{
				...
				else if(currentChild.getNodeName().equals("resetWhenStuck"))
					resetWhenStuck = Boolean.parseBoolean(currentChild.getTextContent());
			}
		...
		if(resetWhenStuck == null)
			resetWhenStuck = true;
		
		return new FollowBoxSettings(segments, minDistance, maxDistance, maxSpeed, startWayPoint, 
				giveWayDistance, intersectionObservationDistance, minIntersectionClearance, resetWhenStuck);
	}
	...
	public Boolean isODAutoPilot()
	{
		...
		try {
			...
			enabled = dtData.getValue(Layer.SCENARIO, 
					"/scenario:scenario/scenario:driver/scenario:openDrive/scenario:autoPilot", Boolean.class);
	...
	public boolean isAlignRoadWaypoints() 
	{
		return alignRoadWaypoints;
	}
			...
			if(road != null)
			{
				for(ODLaneSection laneSection : road.getLaneSectionList())
				{
					if(laneSection.getS() <= startS && startS <= laneSection.getEndS())
					{
						...
						Vector3f pos = point.getPosition().toVector3f();
						Quaternion rot = new Quaternion().fromAngles(0, rotation, 0);
						
						return new ResetPosition("openDriveStartPosition", pos, rot);
					}
				}
			}
	...
	public ODPosition getOpenDriveTargetPosition(Simulator sim)
	{
		if(targetRoadID != null && !targetRoadID.isEmpty() && targetLane != null)
		{
			if(targetS == null)
				targetS = 0d;
			
			ODRoad road = sim.getOpenDriveCenter().getRoadMap().get(targetRoadID);
			if(road != null)
			{
				for(ODLaneSection laneSection : road.getLaneSectionList())
				{
