...
public class TrafficLight
{
	...
	private String name;
	private OffroadPositionType openDrivePosition;
	private Integer affectedLane;
	...
	private TrafficLightState crossingState = null;
	

	public TrafficLight(Simulator sim, String trafficLightID, OffroadPositionType openDrivePosition, 
			Integer affectedLane, String trafficLightTriggerID, String trafficLightPhaseTriggerID,
			String trafficLightGroupID, TrafficLightState initialState, TrafficLightDirection direction,
			int phasePosition, ArrayList<String> requiresRedStringList, TrafficLightPositionData positionData)
	{
		this.sim = sim;
		this.name = trafficLightID;
		this.openDrivePosition = openDrivePosition;
	...
	/**
	 * Returns the OpenDRIVE position (if available)
	 * 
	 * @return
	 * 			OpenDRIVE position or null (if not available)
	 */
	public OffroadPositionType getOpenDrivePosition()
	{
		return openDrivePosition;
	}
	
	
	/**
	 * Returns the OpenDRIVE lane number which is affected by the traffic light (if available)
	 * 
	 * @return
	 * 			Affected OpenDRIVE lane number or null (if not available)
	 */
	public Integer getAffectedLane()
	{
		return affectedLane;
	}
	...
	/**
	 * Getter for the internal program of this traffic light - if available.
	 * 
	 * @return
	 * 			internal program of this traffic light. null if not available
	 */
	public TrafficLightInternalProgram getTrafficLightInternalProgram()
	{
		try {
			return sim.getTrafficLightCenter().getInternalProgram(this);
	...
	/**
	 * Sets the given state to field "crossingState". This can be used to record the current
	 * light state. Value can be retrieved by getCrossingState() in case e.g. the steering 
	 * car has driven through this traffic light.
	 * 
	 * @param crossingState
	 * 			traffic light state
	 */
	public void setCrossingState(TrafficLightState crossingState)
	{
	...
	/**
	 * Gets the value of field "crossingState". Make sure to set this value before using 
	 * "setCrossingState()".
	 * 
	 * @return
	 * 			value of of field "crossingState"
	 */
	public TrafficLightState getCrossingState()
	{
	...
	/**
	 * Returns the elapsed time since the beginning o fthe current state
	 * 
	 * @return
	 * 			time since beginning of the state (in ms)
	 */
	public long getTimeSinceLastStateChange()
	{
		TrafficLightInternalProgram internalProgram = getTrafficLightInternalProgram();
		if(internalProgram != null)
			return internalProgram.getTimeSinceLastStateChange(this);
		else
			return -1;
	}
