...
public class TrafficLightInternalProgram extends Thread
{
			...
			List<TrafficLight> allTrafficLightsList, LinkedList<TrafficLightPhase> intersectionPhasesList) 
	{
		...
		if(intersectionPhasesList != null)
		{
			...
			phase = gotoNextPhase();
			phase.activate(sim.getBulletAppState().getElapsedMilliSecondsSinceStart());
	 ...
	 * 			Current phase
	 */
	public synchronized TrafficLightPhase getCurrentPhase()
	{
		return phase;
	}
	...
	public synchronized int getRemainingRed(TrafficLight trafficLight) throws NeverGreenException, IsGreenException
	{
		...
		// get remaining time (in ms) for current phase
		long remainingRed = phase.timeToExpiration(sim.getBulletAppState().getElapsedMilliSecondsSinceStart());
	...
	public synchronized int getRemainingGreen(TrafficLight trafficLight) throws AlwaysGreenException, IsNotGreenException
	{
		...
		// get remaining time (in ms) for current phase
		long remainingGreen = phase.timeToExpiration(sim.getBulletAppState().getElapsedMilliSecondsSinceStart());
	...
	{		
		while (!stoprequested) 
		{
			...
			if(!sim.isPause())
			{
				...
				else if(currentMode == TrafficLightMode.BLINKING && (timeOfLastBlink + blinkingIntervall <= sim.getBulletAppState().getElapsedMilliSecondsSinceStart()))
				{
					runBlinkingMode();
					timeOfLastBlink = sim.getBulletAppState().getElapsedMilliSecondsSinceStart();
				}
	...
	private void runProgramMode() 
	{
		...
		long currentTime = sim.getBulletAppState().getElapsedMilliSecondsSinceStart();
	...
	/**
	 * Computes the traffic light phase that precedes the given one. If the given 
	 * phase is the first one in the list, the last phase will be returned instead.
	 * 
	 * @param phase
	 * 			Traffic light phase to determine the predecessor of
	 * @return
	 * 			Predecessor phase of the given phase
	 */
	private TrafficLightPhase getPreviousPhase(TrafficLightPhase phase) 
	{
		int index = intersectionPhasesList.lastIndexOf(phase);		
		int size  = intersectionPhasesList.size();
		
		int previousIndex = (size+index-1) % size;

		return intersectionPhasesList.get(previousIndex);
	}
	...
	/**
	 * Stops the loop for the given number of seconds (for TRIGGER mode only)
	 * 
	 * @param seconds
	...
	{
		// wait only if TRIGGER mode is still running
	...
	/**
	 * Computes a forecast of the internal program of the given traffic light.
	 * The forecast contains information about the next three states and their 
	 * times of activation.
	 * 
	 * @param trafficLight
	 * 			traffic light to compute forecast
	 * 
	 * @return
	 * 			forecast containing information about the next three states and
	 * 			their times of activation
	 */
	public TrafficLightForecast computeForecast(TrafficLight trafficLight)
	{
		TrafficLightForecast trafficLightForecast = new TrafficLightForecast();
		
		// get remaining time (in ms) for current phase
		long remainingTime = phase.timeToExpiration(sim.getBulletAppState().getElapsedMilliSecondsSinceStart());
		...
		// go through phases until different state is reached
		while(nextPhase.getState(trafficLight) == phase.getState(trafficLight))
		{
			// if start phase was reached again --> break the cycle, since no change at all
			if(nextPhase.getID().equals(startPhaseID))
				return trafficLightForecast;

			// add duration of this phase and continue with next one
			remainingTime += nextPhase.getDuration();
			nextPhase = getNextPhase(nextPhase);
		}

		// convert ms to steps of 0.1 seconds
		long firstChangeTimeLong = (long) (remainingTime/100);
		trafficLightForecast.setFirstChangeTime((double) (firstChangeTimeLong/10.0));
		
		TrafficLightState firstNextState = nextPhase.getState(trafficLight);
		trafficLightForecast.setFirstNextState(firstNextState);
		
		// go through phases until different state is reached
		while(nextPhase.getState(trafficLight) == firstNextState)
		{
			// add duration of this phase and continue with next one
			remainingTime += nextPhase.getDuration();
			nextPhase = getNextPhase(nextPhase);
		...
		// convert ms to steps of 0.1 seconds
		long secondChangeTimeLong = (long) (remainingTime/100);
		trafficLightForecast.setSecondChangeTime((double) (secondChangeTimeLong/10.0));
		
		TrafficLightState secondNextState = nextPhase.getState(trafficLight);
		trafficLightForecast.setSecondNextState(secondNextState);
		
		// go through phases until different state is reached
		while(nextPhase.getState(trafficLight) == secondNextState)
		{
			// add duration of this phase and continue with next one
			remainingTime += nextPhase.getDuration();
			nextPhase = getNextPhase(nextPhase);
		...
		// convert ms to steps of 0.1 seconds
		long thirdChangeTimeLong = (long) (remainingTime/100);
		trafficLightForecast.setThirdChangeTime((double) (thirdChangeTimeLong/10.0));
		
		TrafficLightState thirdNextState = nextPhase.getState(trafficLight);
		trafficLightForecast.setThirdNextState(thirdNextState);
	...
	/**
	 * Computes the elapsed time since the last state change of the given 
	 * traffic light.
	 * 
	 * @param trafficLight
	 * 			Traffic light to compute the elapsed time of the current 
	 * 			state.
	 * 
	 * @return
	 * 			Time (in ms) since the beginning of the current state. If
	 * 			there is no preceding state, the elapsed time since the 
	 * 			beginning of the simulation will be returned instead.
	 */
	public long getTimeSinceLastStateChange(TrafficLight trafficLight)
	{
		long timeSinceStart = sim.getBulletAppState().getElapsedMilliSecondsSinceStart();
		
		long elapsedTime = phase.timeSinceActivation(timeSinceStart);
		
		// remember start phase to avoid cycles
		String startPhaseID = phase.getID();
		
		// compute previous phase
		TrafficLightPhase previousPhase = getPreviousPhase(phase);

		// go through phases until different state is reached
		while(previousPhase.getState(trafficLight) == phase.getState(trafficLight))
		{
			// if start phase was reached again --> break the cycle, since no change at all
			if(previousPhase.getID().equals(startPhaseID))
				return timeSinceStart;

			// add duration of this phase and continue with previous one
			elapsedTime += previousPhase.getDuration();
			previousPhase = getPreviousPhase(previousPhase);
		}
