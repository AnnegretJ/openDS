...
public class PanelCenter
{
	...
	public static void init(SimulationBasics simBasics)
	{
		sim = simBasics;
		messageBoxGUI = new MessageBoxGUI(sim);
	}
	...
	public static void init(Simulator simulator)
	{
		...
		boolean showAnalog = settingsLoader.getSetting(Setting.General_showAnalogIndicators, true);
        ...
        startX = Simulator.getDrivingTask().getSettingsLoader().getSetting(Setting.OculusRift_panelPosX, startX);
        startY = Simulator.getDrivingTask().getSettingsLoader().getSetting(Setting.OculusRift_panelPosY, startY);
...
//        	guiNode.attachChild(geom);
	...
	public static void update() 
	{
		...
		if(fixRPM != 0)
			...
			setRPMIndicator(car.getCarControl().getRPM()); 
		
		if(resolutionHasChanged && (++updateDelayCounter%2==0))
		{
			resetPanelPosition(car.getCarControl().isAutomatic());
			resetMessageBox();
	...
	private static void updateMaritimePanel(SteeringCar car) 
	{
        ...
        if(maritimeDisplayMode == MaritimeDisplayMode.Panel || maritimeDisplayMode == MaritimeDisplayMode.All)
        {
	        ...
	        maritimeLongitudeText.setText("Laenge: " + df3.format(car.getGeoPosition().getY()) + " O");
        ...
        if(maritimeDisplayMode == MaritimeDisplayMode.MultiFunctionDisplay || maritimeDisplayMode == MaritimeDisplayMode.All)
        {
	        cogText.setText(df4.format(Math.round(car.getHeadingDegree()))+"o");
	        sogText.setText(df.format(car.getCurrentSpeedKmh()/1.852f));
	        magText.setText(df4.format((Math.round(car.getHeadingDegree())+3)%360)+"o");
    ...
    private static String getDegreeText(double decimalValue)
    {
        ...
        return degree + "o" + minutesFormatted + "'";
    }
